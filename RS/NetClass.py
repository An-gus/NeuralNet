...
class Net(object):
    ...
    def __init__(self, lays, scaler=None):
        self.nlays = len(lays)
        self.sizes = lays
    ...
    def load(self, file):
        [self.nlays, self.sizes, self.weights, self.biases, self.scaler] = pickle.load(open(file, 'rb'))

    def save(self, file):
        with open(file, 'wb') as f:
            pickle.dump([self.nlays, self.sizes, self.weights, self.biases, self.scaler], f)
            f.close()
